...
# 10 个真实任务，每个有标准答案
trainset = [
    {"task": "找出 2024 年诺贝尔物理学奖得主", "answer": "Hinton 和 Hopfield"},
    {"task": "查询今天上海天气",               "answer": "..."},
    {"task": "搜索 Python 3.12 的新特性",      "answer": "..."},
    # ...
]

Step 2：Agent 执行任务，收集 trace


Step 3. 一个 reflection LLM 读取所有 trace，输出自然语言诊断
例如：
诊断报告：
   1. Agent 不知道应该用英文 query 效果更好（第1个任务走了弯路）
   2. 工具描述没说明返回的是实时数据还是缓存数据（第2个任务用了过期数据）
   3. 没有说明 query 的最佳格式和长度建议
   4. 没有说明如何处理返回结果为空的情况

建议修改方向：在描述中加入 query 语言建议、数据时效说明、参数最佳实践。
Step 4. LLM 根据诊断，提出一个 mutated 候选
# 候选 v2（mutation 结果）
tool_description_v2 = """
search(query: str) -> list[SearchResult]:
  ...
  参数:
    query: 搜索关键词。建议用英文可获得更广泛结果；
           中英文均可，长度建议 3-10 词。

  返回: 最新抓取的网页摘要列表（实时数据，非缓存）。
        若结果不理想，尝试换用同义词或英文重试。

  示例:
    search("Nobel Prize Physics 2024 winner")
    search("Shanghai weather today realtime")
"""

Step 5：快速筛选 + Pareto 评估
minibatch 评分:
  v1: 2/3 正确
  v2: 3/3 正确  ← 有提升，进入 Pareto 候选池
Pareto 表:
              task1  task2  task3  task4 ... 总分
  v1 (原版)    ✓      ✗      ✓      ✓       6/10
  v2 (mutation)✓      ✓      ✓      ✓       8/10

Step 6：下一轮迭代 —— Crossover
Pareto 表（迭代3次后）:
              时事任务  技术任务  总分
  v2            ✓✓✓      ✓✓       8/10   ← 时事强
  v3            ✓✓       ✓✓✓      7/10   ← 技术强
...
让 LLM 把 v2 和 v3 的优点合并：
# v4 = crossover(v2, v3)
tool_description_v4 = """
search(query: str, lang: str = "auto") -> list[SearchResult]:
  实时搜索网页。

  时事/新闻类查询：用中文或英文均可，加上年份更精准
    例: search("2024 诺贝尔物理学奖")
  
  技术/文档类查询：优先用英文，加版本号
    例: search("Python 3.12 new features PEP")
    
  返回实时数据。结果为空时换同义词重试。
"""
# 评分: 9/10，成为新的 Pareto 最优
Step 7：输出最终结果
经过 N 轮迭代后，GEPA 输出最优的工具描述，自动写回 Hermes Agent 的配置
